...
//                    System.out.println(group.level + " " + group.name + " " + course.toString());
        ...
        System.out.println("共排课 " + amount + "个");
    }
    ...
    public static boolean checkLocWeekTime(Group group, Record record, int locWeek, int weekDay, int time) {
        ...
        //此时间段 本课程 无可用教室
        if (!checkHasClassRoom(record, locWeek, weekDay, time)) {
            return false;
        }
        //此时间 无可用老师
        if (!checkHasTeacher(record, locWeek, weekDay, time)) {
            return false;
        }
        return true;
    }
     ...
     * 此时间段，是否有可用教室
     */
    public static boolean checkHasClassRoom(Record record, int locWeek, int weekDay, int time) {
        for (Integer classroom : record.classroomList) {
            if (checkClassroom(classroom, locWeek, weekDay, time)) {
                return true;
            }
        }
        return false;
    ...
    public static void setClassroom(Record record, Course course) {
        for (Integer classroom : record.classroomList) {
            if (checkClassroom(classroom, course.locWeek, course.weekDay, course.time)) {
     ...
     * 此时间段，是否有可用老师
     */
    public static boolean checkHasTeacher(Record record, int locWeek, int weekDay, int time) {
        for (String teacherName : record.teacherList) {
            if (TextUtil.notEmpty(teacherName)) {
                if (checkTeacher(teacherName, locWeek, weekDay, time)) {
                    return true;
                }
            }
        }
    ...
    public static void setTeacher(Record record, Course course) {
        for (String teacherName : record.teacherList) {
            if (TextUtil.notEmpty(teacherName)) {
                if (checkTeacher(teacherName, course.locWeek, course.weekDay, course.time)) {
     ...
     * 校验教室是否可用
     */
    public static boolean checkClassroom(int classroom, int locWeek, int weekDay, int time) {
        for (Group group : groupList) {
            for (List<Course> courseList : group.courseMap.values()) {
                for (Course tempCourse : courseList) {
                    if (tempCourse.classroom == classroom &&
                            tempCourse.locWeek == locWeek &&
                            tempCourse.weekDay == weekDay &&
                            tempCourse.time == time) {
                        return false;
                    }
                }
            }
     ...
     * 校验老师是否可用
     */
    public static boolean checkTeacher(String teacherName, int locWeek, int weekDay, int time) {
        for (Group group : groupList) {
            for (List<Course> courseList : group.courseMap.values()) {
                for (Course tempCourse : courseList) {
    ...
    public static List<Group> initData() {
        ...
        List<Record> recordList18 = dealCourseList(excelList18);

        List<List<String>> excelList19 = ExcelUtils.readFromXLSX(corse19);
        List<Record> recordList19 = dealCourseList(excelList19);

        List<List<String>> excelList20 = ExcelUtils.readFromXLSX(corse20);
        List<Record> recordList20 = dealCourseList(excelList20);
     ...
     * 处理课程数据
     *
     * @param excelList 原始表格数据
     * @return 原始数据
     */
    public static List<Record> dealCourseList(List<List<String>> excelList) {
